#!/usr/bin/env python3
"""
优化版数据导入脚本 - 将课程评价数据快速导入到评课社区现有课程中
"""
import os
import sys
import django
...
def build_course_index():
    ...
    return course_index, teacher_index


def find_matching_course(course_name, teacher_name, course_index, teacher_index):
    """在现有课程中查找匹配的课程（优化版）"""
    # 首先尝试精确匹配
    exact_key = f"{course_name}|{teacher_name}"
...
def import_reviews():
    ...
    # 构建课程索引
    course_index, teacher_index = build_course_index()
    
    # 获取现有评价数据（用Set来快速查重）
    existing_review_signatures = set()
    for review in Review.objects.select_related('course', 'course__main_teacher').all():
        signature = f"{review.course.id}|{review.comment[:50]}"  # 用课程ID和评论前50字符作为签名
    ...
    with open(csv_file_path, 'r', encoding='utf-8') as csvfile:
        ...
        for i, row in enumerate(reader):
            try:
                ...
                # 查找匹配的课程
                course = find_matching_course(course_name, teacher_name, course_index, teacher_index)
                if not course:
                ...
                existing_review_signatures.add(signature)  # 添加到已存在集合中
