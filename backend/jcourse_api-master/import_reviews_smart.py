...
def find_smart_matching_course(course_name, teacher_name):
    """智能匹配课程"""
    ...
    # 标准化课程名称
    normalized_course = normalize_course_name(cleaned_course)
    
    # 匹配策略列表
    strategies = [
        # 策略1: 精确匹配标准化后的名称
        {'course': normalized_course, 'teacher': cleaned_teacher, 'method': 'exact'},
        # 策略2: 精确匹配清理后的名称
        {'course': cleaned_course, 'teacher': cleaned_teacher, 'method': 'exact'},
        # 策略3: 精确匹配原始名称
        {'course': course_name, 'teacher': teacher_name, 'method': 'exact'},
        # 策略4: 模糊匹配 - 课程名包含关系
        {'course': normalized_course, 'teacher': cleaned_teacher, 'method': 'contains'},
        {'course': cleaned_course, 'teacher': cleaned_teacher, 'method': 'contains'},
    ]
    
    for strategy in strategies:
        course_variant = strategy['course'].strip()
        teacher_variant = strategy['teacher'].strip()
        method = strategy['method']
        
        if not course_variant or not teacher_variant:
            continue
        ...
        if method == 'exact':
            ...
            # 包含匹配
            course = Course.objects.filter(
                name__icontains=course_variant,
                main_teacher__name=teacher_variant
            ).first()
        
        if course:
            print(f"匹配成功: {course_name} -> {course.name} ({course.main_teacher.name}) [策略: {method}]")
            return course
...
def import_reviews_smart():
    ...
    with open(csv_file_path, 'r', encoding='utf-8') as csvfile:
        ...
        for i, row in enumerate(reader):
            try:
                ...
                # 智能匹配课程
                course = find_smart_matching_course(course_name, teacher_name)
                if not course:
                    not_found_count += 1
                    not_found_details.append({
                        ...
                        'cleaned_course': clean_course_name(course_name),
                        'cleaned_teacher': clean_teacher_name(teacher_name),
                        'normalized_course': normalize_course_name(clean_course_name(course_name))
                    })
                    continue
    ...
    if not_found_details:
        ...
        for i, detail in enumerate(not_found_details[:10]):
            ...
            print(f"    标准化: {detail['normalized_course']} - {detail['cleaned_teacher']}")


if __name__ == '__main__':
    print("开始智能匹配导入课程评价数据...")
    import_reviews_smart()
    print("智能导入完成！")